...
// Else if we get at least URL parameter AND the cookie exist
	...
	if (pate_cookie_choco['source'] != undefined) {
		if (
			url_src !== false &&
			pate_cookie_choco['source'].indexOf(url_src) != -1
		) {
	...
	if (pate_cookie_choco['medium'] != undefined) {
		if (
			url_mdm !== false &&
			pate_cookie_choco['medium'].indexOf(url_mdm) != -1
		) {
	...
	if (pate_cookie_choco['campaign'] != undefined) {
		if (
			url_cpn !== false &&
			pate_cookie_choco['campaign'].indexOf(url_cpn) != -1
		) {
	...
	if (pate_cookie_choco['purl'] != undefined) {
		if (
			url_purl !== false &&
			pate_cookie_choco['purl'].indexOf(url_purl) != -1
		) {
...
//const ref_ID = read_cookie('ref_ID')
